    ...
    public sealed class VRTK_SDKManager : MonoBehaviour
    {
        ...
        public void TryLoadSDKSetupFromList(bool tryUseLastLoadedSetup = true)
        {
            ...
            else if (UnityEngine.VR.VRSettings.enabled)
            {
                ...
                index = Array.FindIndex(
                    setups,
                    setup => setup.usedVRDeviceNames.Contains(UnityEngine.VR.VRSettings.loadedDeviceName)
                );
            }
            else
            {
        ...
        public void TryLoadSDKSetup(int startIndex, bool tryToReinitialize, params VRTK_SDKSetup[] sdkSetups)
        {
            ...
            bool isDeviceAlreadyLoaded = UnityEngine.VR.VRSettings.enabled
                                         && sdkSetups[0].usedVRDeviceNames.Contains(UnityEngine.VR.VRSettings.loadedDeviceName);
            if (!isDeviceAlreadyLoaded)
            {
                if (!tryToReinitialize && !UnityEngine.VR.VRSettings.enabled && !string.IsNullOrEmpty(UnityEngine.VR.VRSettings.loadedDeviceName))
                {
                    sdkSetups = sdkSetups.Where(setup => !setup.usedVRDeviceNames.Contains(UnityEngine.VR.VRSettings.loadedDeviceName))
                                         .ToArray();
                }

                VRTK_SDKSetup[] missingVRDeviceSetups = sdkSetups
                    .Where(setup => setup.usedVRDeviceNames.Except(UnityEngine.VR.VRSettings.supportedDevices).Any())
                    .ToArray();
                foreach (VRTK_SDKSetup missingVRDeviceSetup in missingVRDeviceSetups)
                {
                    string missingVRDevicesText = string.Join(
                        ...
                        missingVRDeviceSetup.usedVRDeviceNames
                                            .Except(UnityEngine.VR.VRSettings.supportedDevices)
                                            .ToArray()
                    );
                ...
                string[] vrDeviceNames = sdkSetups
                    ...
                    .ToArray();
                UnityEngine.VR.VRSettings.LoadDeviceByName(vrDeviceNames);
            }
...
#pragma warning restore 618
        ...
        private IEnumerator FinishSDKSetupLoading(VRTK_SDKSetup[] sdkSetups, VRTK_SDKSetup previousLoadedSetup)
        {
            ...
            string loadedDeviceName = string.IsNullOrEmpty(UnityEngine.VR.VRSettings.loadedDeviceName) ? "None" : UnityEngine.VR.VRSettings.loadedDeviceName;
